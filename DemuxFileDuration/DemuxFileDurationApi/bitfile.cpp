...
void bit_file_c::SeekFileFromLast(uint32_t pos)
{
	m_InStream->seekg(pos, ios::beg);
}
bool bit_file_c::OpenFile(const char *fileName, long &fileSize)
{
	if (m_InStream != NULL)
	{
		return false;
...
void bit_file_c::AllocateFifoBuffer(int size)
{
	pFileBuffer = new uint8_t[size];
 
}

bool bit_file_c::ReadChunkFromFile( int chunkSize)
{

	m_InStream->read((char *)pFileBuffer, (uint32_t)chunkSize);
	int res = m_InStream->gcount();
	if (res != chunkSize)
		return false;
...
bool bit_file_c::ReadChunkFromFile(uint8_t *buffer, int chunkSize)
{
	
	m_InStream->read((char *)buffer, (uint32_t)chunkSize);
	return true;
}
