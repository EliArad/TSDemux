...
class bit_file_c
{
    ...
    public:
		...
		int CheckBit(void);
		bool OpenFile(const char *fileName, long &fileSize);
		int GetBitCount();
		int CheckByte(void);
		void ResetBitCount();
		void AllocateFifoBuffer(int size);
		bool ReadChunkFromFile(uint8_t *buffer, int chunkSize);
		bool ReadChunkFromFile(int chunkSize);
		void SeekFileFromLast(uint32_t pos);
		void CloseFile();
